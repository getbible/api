...
import {rankingFilters, activeJob, editTrafficFilter} from './analytics.js';
const pages = ['Overview', 'Traffic', 'Audience', 'Translations', 'Resources', 'Events', 'Manage', 'Sessions'];
...
function App() {
    ...
    return <div className="console"><aside className={`sidebar ${navOpen ? 'expanded' : ''}`}><a className="brand" href="#overview" onClick={e => { e.preventDefault(); setPage('Overview'); }}><img src="/favicon.png" alt=""/><span>getBible<small>OPERATIONS</small></span></a><div className="workspace-label">SERVER CONSOLE</div><nav aria-label="Main navigation">{pages.map((label, i) => <button className={page === label ? 'active' : ''} aria-current={page === label ? 'page' : undefined} key={label} aria-label={label} onClick={() => { setPage(label); if (label === 'Manage') setManagementSection(''); setNavOpen(false); }}><span className="nav-number">{String(i + 1).padStart(2, '0')}</span>{label}</button>)}</nav><div className="sidebar-bottom"><span className="eyebrow">CONSOLE STATE</span><strong>{wake === 'ready' ? 'Connected' : wake === 'unavailable' ? 'History unavailable' : 'Initializing'}</strong><small>Traffic is recorded continuously.</small><button className="btn btn-sm btn-outline-secondary" onClick={async () => { try {
