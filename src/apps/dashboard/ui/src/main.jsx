...
function Translations({ onError, execute, onDetail, refresh }) {
    ...
    <Panel title="Translation memory" detail="Warm on demand, retain within the configured lifetime and memory budget.">
      <div className="filter-grid">
        <label>Runtime endpoint<select aria-label="Runtime endpoint" className="form-select" value={selected} onChange={e => setSelected(e.target.value)}><option value="">Select endpoint</option>{endpoints.map(e => <option key={`${e.domain}/${e.label}`} value={`${e.domain}/${e.label}`}>{e.domain} / {e.label} · {e.kind}</option>)}</select></label>
...
function OperationField({field, values, setValues, onError}) {
    ...
    if (field.type === 'file') {
        ...
        input = <select aria-label={field.label || field.name} className="form-select" required={field.required && !field.allow_empty} value={values[field.name] ?? ''} onChange={e => change(e.target.value)}><option value="">Select…</option>{(field.choices || field.enum).map(c => <option key={c} value={c}>{c}</option>)}</select>;
    } else {
...
function Management({ execute, onError, onDetail, refresh }) {
    ...
    return <>{management?.refresh?.pending && <div className={`alert ${management.refresh.state === 'failed' ? 'alert-warning' : 'alert-info'}`} role="status"><strong>{management.refresh.state === 'failed' ? 'Management service update needs attention' : 'Management services are updating'}</strong><div>{management.refresh.state === 'failed' ? management.refresh.last_error || 'The service refresh could not be scheduled. Management remains available; retry the update or inspect the service from the CLI.' : 'Accepted jobs will finish before management restarts. Existing results and issued credentials remain available.'}</div></div>}<Panel title="Server management" detail="Operations use the same validated commands as the CLI. Started jobs continue when this page closes."><form className="management-form" onSubmit={submit}><label>Operation<select aria-label="Operation" className="form-select" value={operation} onChange={e => { setOperation(e.target.value); const item = operations.find(o => (o.id || o.operation || o.name) === e.target.value); setValues(Object.fromEntries((item?.fields || []).filter(f => f.default !== undefined).map(f => [f.name, f.default]))); }}><option value="">Select an operation</option>{operations.map(item => <option key={item.id || item.operation || item.name} value={item.id || item.operation || item.name}>{item.title || item.label || item.name || item.id}</option>)}</select></label>{spec?.description && <p className="text-secondary">{spec.description}</p>}<div className="filter-grid">{(Array.isArray(fields) ? fields : Object.entries(fields).map(([name, f]) => ({ name, ...f }))).map(field => <OperationField key={field.name} field={field} values={values} setValues={setValues} onError={onError}/>)}</div><button className="btn btn-primary" disabled={!spec || paused}>Review operation</button></form>{catalogue && !operations.length && <Empty>No management operations were returned by the broker.</Empty>}</Panel><Panel title="Operation history" detail="Select a job to view progress and output">{jobs ? <DataTable rows={array(jobs, 'jobs')} onSelect={async (row) => { try {
