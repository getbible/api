...
check "sync timer monthly"       "OnCalendar=monthly"      "$(cat "$SB/etc/systemd/system/getbible-sync-static_example_test-v2.timer")"
check "telemetry rotation rendered" "telemetry owns"       "$(cat "$SB/etc/getbible/logrotate.conf")"
