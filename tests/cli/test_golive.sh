...
check "no DNS switch with incomplete API profile" "" "$(grep '^dns ' "$CF_LOG" || true)"
check "no DNS switch recorded"    "" "$(grep 'CLOUDFLARE_DNS_AT=' "$SB/var/lib/getbible/state/$D8/state.conf" || true)"
check "rules failure reported"    "Cloudflare DNS was NOT updated" "$OUT"
