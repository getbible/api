...
- **Static domains**: each endpoint a tree of JSON, checksum and text files
  synchronised from its own git repository by an isolated user with its own
...
- **Pages and OpenAPI**: every endpoint has a documentation page at `/vN/`
  and an OpenAPI document at `/vN/openapi.json`, the domain page at `/` lists
  ...
  endpoints, taken straight from the repository.
