...
Set up root's deploy key and SSH configuration as in
[INSTALL.md](INSTALL.md#1-clone), then:
...
before DNS changes. System > Check this host shows what was found. Later,
`sudo ./getbible.sh self-update` brings the manager up to date with the same
key; it does not touch hosted domains (see [UPDATING.md](UPDATING.md)).
