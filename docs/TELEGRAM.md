...
| domains | deployed, removed, access mode or limits changed, endpoint added or removed, default endpoint changed |
...
| staging and go-live | domain staged; live (certificate, DNS outcome, verification); go-live failed at the certificate or at activation; staged again |
| tokens | issued (label), revoked (id) |
| log rotation | archive count per domain, warning at the retention ceiling |
